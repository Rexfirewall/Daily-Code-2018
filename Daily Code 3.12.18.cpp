...
int main()
{
	...
	ALLEGRO_BITMAP *darkPic = al_load_bitmap("dark.jpeg");
	element air;
	element water;
	element fire;
	element earth;
	...
	dark.initElement(350, 600, 5, darkPic);
	...
	while (1)
	{
		...
		if (ev.type == ALLEGRO_EVENT_TIMER) {
			...
			if (dark.clicked(bouncer_x, bouncer_y))
				if (dark.isLifted() == false)
					dark.liftUp();
				else
					dark.PutDown();
		}
